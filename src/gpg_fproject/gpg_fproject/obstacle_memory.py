...
class ObstacleMemoryNode(Node):
    ...
    def verify_duplicated_obstacle(self, x_get_time_step, y_get_time_step, z_get_time_step):
        ...
        distance_eucluiden = np.sqrt((x_get_time_step - self.x_coords)**2 + 
                                     (y_get_time_step - self.y_coords)**2 + 
                                     (z_get_time_step - self.z_coords)**2)
        return np.any(distance_eucluiden < self.duplicate_threshold)
    ...
    def publish_obstacle_memory(self):
        ...
        arraye_format = np.array(obstacle_msg.float_storage).reshape(-1, 3)
...
# for i, obs_coords in enumerate(obstacle_msg.float_storage[:3]): 
        #     topic_name = f'/obstacle_position_{i}'
        #     if topic_name not in self.publishers:
        #         self.publishers[topic_name] = self.create_publisher(PointStamped, topic_name, 10)
